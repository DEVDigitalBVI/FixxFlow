...
export default async function TicketsPage({ searchParams }: { searchParams: Promise<Filters> }) {
  ...
  const activeFilterCount = [filters.status, filters.priority, filters.team, filters.overdue, filters.sla].filter(Boolean).length;
  ...
  return <div className="page">{usage}<ConversationRefresh/><header className="page-header"><div><span className="page-eyebrow">IT support</span><h1>Ticket queue</h1><p>Find, assign, and move support work forward. SLA targets use calendar hours.</p></div><Link className="button button-primary" href="/app/tickets/new">New ticket</Link></header>
    ...
    <form className="queue-filters"><input type="hidden" name="view" value={view}/><label>Search<input className="input" name="q" defaultValue={filters.q} type="search" maxLength={200} aria-describedby="ticket-search-hint" placeholder="VPN error 809 or #1052"/></label><details className="queue-filter-options" open={activeFilterCount > 0 || sort !== "updated"}><summary>Filters and sort{activeFilterCount > 0 ? ` · ${activeFilterCount} active` : ""}</summary><div className="queue-filter-fields"><label>Status<select className="input" name="status" defaultValue={filters.status ?? ""}><option value="">Any status</option>{Object.entries(ticketStatuses).map(([value, p]) => <option key={value} value={value}>{p.label}</option>)}</select></label><label>Priority<select className="input" name="priority" defaultValue={filters.priority ?? ""}><option value="">Any priority</option>{Object.entries(ticketPriorities).map(([value, p]) => <option key={value} value={value}>{p.label}</option>)}</select></label><label>Team<select className="input" name="team" defaultValue={filters.team ?? ""}><option value="">All teams</option>{teams?.map(team => <option key={team.id} value={team.id}>{team.name}</option>)}</select></label><label>Sort by<select className="input" name="sort" defaultValue={sort}>{Object.entries(sortOptions).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="queue-check"><input type="checkbox" name="overdue" value="1" defaultChecked={filters.overdue === "1"}/> Manual date overdue</label><label className="queue-check"><input type="checkbox" name="sla" value="breached" defaultChecked={filters.sla === "breached"}/> SLA breached</label></div></details><button className="button button-secondary" type="submit">Apply filters</button><Link href={`/app/tickets?view=${view}`} className="queue-clear">Clear filters</Link></form>
