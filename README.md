...
Production Auth uses `https://www.fixxflow.com`, confirmed email signup, an eight-character minimum, and leaked-password protection. Leaked-password protection is managed in the hosted Auth dashboard. Keep `NEXT_PUBLIC_SITE_URL` aligned with this domain. The server secret is only required for organization bootstrap and invitations; never expose it to the browser.
